...
elasticsearch_chart = Helm(os.path.dirname(
    __file__), chart_name="elasticsearch")
...
values_file_object['replicas'] = elasticsearch_chart.execution.get_local_parameter(
    'REPLICAS')
values_file_object['minimumMasterNodes'] = elasticsearch_chart.execution.get_local_parameter(
    'NUMBER_MASTERS')
values_file_object[
        'esMajorVersion'] = elasticsearch_chart.execution.get_local_parameter(
    'ES_VERSION')
if elasticsearch_chart.execution.domain_name:
    values_file_object['ingress']['enabled'] = True
    values_file_object['ingress']['hosts'][0][
        'host'] = f'es-{namespace}.{elasticsearch_chart.execution.domain_name}'
...
elasticsearch_chart.install_chart(release_name="elastic",
                                  ...
                                  additional_values=[
                                      f"--values {execution_file}"],
                                  timeout=360)
services_names = [f'elasticsearch-headdless-{namespace}']
if elasticsearch_chart.execution.get_local_parameter('INSTALL_KIBANA'):
    ...
    if kibana_chart.execution.domain_name:
        values_file_object['ingress']['enabled'] = True
        values_file_object['ingress']['hosts'][0][
            'host'] = f'kibana-{namespace}.{elasticsearch_chart.execution.domain_name}'
    services_names += f'kibana-kibana-{namespace}'
    execution_file = os.path.join(
...
elasticsearch_chart.execution.write_connection_info(service_name="Elasticsearch", ingresses=[
    f'es-{namespace}.{elasticsearch_chart.execution.domain_name}', f'kibana-{namespace}.{elasticsearch_chart.execution.domain_name}'])
